...
const starterMessages = [
  {
    role: 'assistant',
    content:
      'Hi! Choose a local model, load it, and then ask me to generate, debug, refactor, test, or explain code.',
  },
];
...
function populateModels() {
  if (!nodes.modelSelect || !Array.isArray(LOCAL_MODELS) || LOCAL_MODELS.length === 0) {
    return;
  }
...
function updateWebGPUStatus() {
  ...
  if (nodes.webgpuStatus) {
    nodes.webgpuStatus.textContent = supported ? 'Available' : 'Unavailable';
  }

  if (nodes.runtimeStatus) {
    nodes.runtimeStatus.textContent = supported ? 'Browser WebGPU' : 'WebGPU unavailable';
  }

  if (nodes.compatibilityMessage) {
    nodes.compatibilityMessage.textContent = supported
      ? 'WebGPU is available. You can load a local model. Model loading speed depends on your device and available GPU memory.'
      : 'WebGPU is unavailable. Use a recent Chromium-based browser with WebGPU enabled.';
    nodes.compatibilityMessage.className = supported
      ? 'compatibility-message success'
      : 'compatibility-message error-text';
  }

  if (nodes.loadModelButton) {
    nodes.loadModelButton.disabled = !supported;
  }

  if (!supported) {
    setChatEnabled(false);
    if (nodes.status) nodes.status.textContent = 'WebGPU unavailable';
  }
}
...
function handleModelSelection() {
  ...
  if (nodes.modelName && getLoadedModelId() !== selected.id) {
    nodes.modelName.textContent = 'Not loaded';
  }

  if (!isModelReady && nodes.status) {
    nodes.status.textContent = hasWebGPU() ? 'Load a model first' : 'WebGPU unavailable';
  }
}
...
async function handleModelLoad() {
  ...
  if (!hasWebGPU()) {
    pushError('WebGPU is unavailable in this browser or device.');
    return;
  }
  ...
  updateProgress(0, 'Starting model loader…');
  ...
  try {
    ...
    if (nodes.modelName) nodes.modelName.textContent = loaded.name;
    if (nodes.runtimeStatus) nodes.runtimeStatus.textContent = 'Local WebGPU';
    if (nodes.status) nodes.status.textContent = 'Ready — local model';
    if (nodes.modelHint) nodes.modelHint.textContent = 'Model loaded. Chat generation runs locally on this device.';
...
async function handleClearCache() {
  if (!window.confirm('Clear locally cached model data for this site? You may need to download the model again.')) {
    return;
  }
  ...
  try {
    ...
    if (nodes.modelName) nodes.modelName.textContent = 'Not loaded';
    if (nodes.runtimeStatus) nodes.runtimeStatus.textContent = hasWebGPU() ? 'Browser WebGPU' : 'WebGPU unavailable';
    if (nodes.status) nodes.status.textContent = hasWebGPU() ? 'Load a model first' : 'WebGPU unavailable';
  } catch (error) {
    pushError(error instanceof Error ? error.message : 'Could not clear local model cache.');
  }
}
...
async function sendPrompt(rawPrompt, options = {}) {
  ...
  isGenerating = true;
  if (nodes.status) nodes.status.textContent = 'Generating locally…';
  if (nodes.sendButton) nodes.sendButton.disabled = true;
  ...
  try {
    ...
    assistantMessage.content = reply || 'No response was generated.';
  } catch (error) {
    messages.pop();
    ...
    isGenerating = false;
    if (nodes.status) nodes.status.textContent = 'Ready — local model';
    if (nodes.sendButton) nodes.sendButton.disabled = !isModelReady;
    saveMessages();
    renderMessages();
  }
}
...
function renderMessages() {
  ...
  nodes.messages.replaceChildren(...messages.map((message, index) => renderMessage(message, index)));
...
function renderMarkdown(parent, content) {
  ...
  parts.forEach((part, index) => {
    if (index % 2 === 1) renderCodeBlock(parent, part);
    else renderText(parent, part);
  });
}
...
function renderCodeBlock(parent, rawBlock) {
  ...
  const language = firstLineBreak > -1 ? block.slice(0, firstLineBreak).trim() : '';
  const code = firstLineBreak > -1 ? block.slice(firstLineBreak + 1).trim() : block.trim();
...
function applyBasicHighlighting(codeNode, code, language) {
  const supported = new Set(['js', 'jsx', 'ts', 'tsx', 'javascript', 'typescript', 'python', 'py', 'css', 'html']);
  ...
  const pattern = /(\/\/.*|#.*|"(?:\\.|[^"])*"|'(?:\\.|[^'])*'|`(?:\\.|[^`])*`|\b(?:const|let|var|function|return|if|else|for|while|class|import|from|export|async|await|try|catch|def|lambda|yield|true|false|null|None|and|or|not)\b)/g;
  let cursor = 0;

  for (const match of code.matchAll(pattern)) {
    const index = match.index ?? 0;
    if (index > cursor) codeNode.append(document.createTextNode(code.slice(cursor, index)));
  ...
  if (cursor < code.length) codeNode.append(document.createTextNode(code.slice(cursor)));
}
...
function setModelLoading(isLoading) {
  if (nodes.loadModelButton) nodes.loadModelButton.disabled = isLoading || !hasWebGPU();
  if (nodes.modelSelect) nodes.modelSelect.disabled = isLoading;
  if (nodes.clearCacheButton) nodes.clearCacheButton.disabled = isLoading;
}

function setChatEnabled(enabled) {
  if (nodes.prompt) nodes.prompt.disabled = !enabled;
  if (nodes.sendButton) nodes.sendButton.disabled = !enabled || isGenerating;
}

function updateProgress(progress, text) {
  const value = Math.min(1, Math.max(0, Number(progress) || 0));
  if (nodes.downloadProgress) nodes.downloadProgress.value = value;
  if (nodes.downloadPercent) nodes.downloadPercent.textContent = `${Math.round(value * 100)}%`;
  if (nodes.downloadStatus) nodes.downloadStatus.textContent = text || 'Loading model…';
}
...
function clearChat() {
  ...
  saveMessages();
  renderMessages();
  if (isModelReady) nodes.prompt?.focus();
}
...
function getSelectedModel() {
  if (!Array.isArray(LOCAL_MODELS)) return null;
  return LOCAL_MODELS.find((model) => model.id === nodes.modelSelect?.value) || LOCAL_MODELS[0] || null;
}
